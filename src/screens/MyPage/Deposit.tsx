    ...
    return (
        <StyledView>
            ...
            <DepositButtonWrapper>
                {buttonData.map((button) => <DepositButton variant={button.id === selectedButton ? 'default' : 'disabled'} onClick={() => { console.log(button.id); setSelectedButton(button.id) }}>{button.text}</DepositButton>)}
            </DepositButtonWrapper>
            <AmountText>결제 방법</AmountText>
            <VirtualAccountWrapper>
                <VirtualAccountText>가상계좌</VirtualAccountText>
            </VirtualAccountWrapper>
            <WarningText>*충전 신청 후 2시간 이내에 입금하지 않으면 충전이 취소됩니다.</WarningText>
            <VirtualAccountGuideView>
                <GuideText>가상 계좌 안내</GuideText>
                <GuideDescriptionText>- 가상계좌를 발급 받아 무통장으로 입금할 수 있어요.</GuideDescriptionText>
                <GuideDescriptionText>- 등록하신 계정의 휴대폰 번호로 무통장 계좌 안내가 발송돼요.</GuideDescriptionText>
            </VirtualAccountGuideView>
        </StyledView>
    )
}
